...
int main(){
	...
	for(int count=0;count<X_train.size();count++){
		cout<<"Test case"<<count<<":"<<endl;
		inputVals = X_train[count];
		
		myNet.feedForward(inputVals);
		cout<<"Input:"<<endl;
		for (int i = 1; i < 28 * 28+1; i++) {
				if(X_train[count][i] == 0){
					cout << "0";
				}else{
					cout << "*";
				}
		...
		cout<<"correct answer:"<<y_train[count]<<endl<<endl;
	}
    ...
    cout << "CPU Running Time : "<< duration.count() << " microseconds" << endl; 
