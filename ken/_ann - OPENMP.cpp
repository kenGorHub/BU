		...
		for(n = 0; n < neuron[layer].size(); n++){
			...
			for(prevN = 0; prevN < neuron[prevLayer].size(); prevN++){			
				...
				if(prevN == neuron[prevLayer].size()-1){
					...
					neuron[prevLayer][prevN].weight[n] = neuron[prevLayer][prevN].weight[n] * momentum - (learingRate * neuron[layer][n].getLoss());
				}else{
					//cout << oldweight << " - " << (learingRate * neuron[layer][n].getLoss() * neuron[prevLayer][prevN].getOutput());
					
					neuron[prevLayer][prevN].weight[n] = neuron[prevLayer][prevN].weight[n] * momentum - (learingRate * neuron[layer][n].getLoss() * neuron[prevLayer][prevN].getOutput());
				}
